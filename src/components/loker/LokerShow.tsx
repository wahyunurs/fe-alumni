...
import { useLokerAdmin, useLokerAlumni, useLokerMitra, Loker } from "@/hooks/loker/useStore.hook";
...
export default function LokerShow({ uuid, show, onDone }: {uuid?: string, show?: boolean, onDone?: () => void}){
    ...
    const { show: getDataLokerMitra } = useLokerMitra();
    ...
    useEffect(() => {
        ...
        if (uuid && role){
            ...
            if (role === 'mitra') getDataLokerMitra(uuid).then((result) => {
                setData(result);    
            })
        }
                    ...
                    <div className="w-full flex flex-col items-start">
                        <div className="w-full text-left">
                            ...
                            <p className="mt-2 text-sm text-gray-800">
                                <span className="font-bold">Tipe kerja:</span> {data?.TipeKerja}
                            </p>
