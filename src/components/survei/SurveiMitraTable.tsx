...
import { useSurveiMitra } from '@/hooks/survei-mitra/surveiMitra.hook';

export default function SurveiMitraTable() {
    const [filter, setFilter] = useState({ currentPage: 1, lastPage: 1 });
    const [refresh, setRefresh] = useState<boolean>(true);
    const { data: surveiMitraData, index: fetchSurveiMitra, pagination } = useSurveiMitra();
            ...
            <section>
                <div className="flex flex-col gap-4 mb-6">
                    <p className="text-sm text-gray-500">
                        Berikut adalah keseluruhan data survei penilaian mitra untuk kinerja alumni.
                    </p>
                </div>
                <div className="overflow-x-auto shadow rounded-lg">
                    <Table hoverable striped>
                        <Table.Head className="text-xs sm:text-sm">
                            ...
                            <Table.HeadCell>Nama Mitra</Table.HeadCell>
                        ...
                        <Table.Body className="divide-y text-xs sm:text-base">
                            {surveiMitraData.length === 0 ? (
                                ...
                                surveiMitraData.map((item, index) => {
                                return (
